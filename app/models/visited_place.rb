...
class VisitedPlace < ApplicationRecord
  has_many :reports, class_name: 'VisitedPlaceReport', foreign_key: 'place_id', inverse_of: :visited_place

  validates :name, presence: true

  def self.recently_visited
    joins('LEFT JOIN visited_place_reports ON visited_places.id = visited_place_reports.place_id').
