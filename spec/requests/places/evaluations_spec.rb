  ...
  describe 'PUT /places/:place_id/evaluations/:id' do
    ...
    context '誤ったリクエストの場合' do
      it '評価を修正できない' do
        ...
        put "/places/#{place.id}/evaluations/#{evaluation.id}", params: { place_evaluation: { visited_on: '2020/01/98', point: 2000 } }
        aggregate_failures do
          expect(response).to have_http_status(:ok)
          expect(response.body).to include('test_place')
          expect(response.body).to include(Date.yesterday.strftime('%Y/%m/%d'))
          expect(response.body).to include('もう行きたくない')
          expect(response.body).not_to include(Date.today.strftime('%Y/%m/%d'))
          expect(response.body).not_to include('また行きたい')
        end
      end
    end
  end
